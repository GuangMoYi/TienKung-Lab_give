...
from legged_lab.assets.tsts2 import TIENKUNG2LITE_CFG
...
@configclass
class LiteRewardCfg:
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_yaw_frame_exp, weight=2.0, params={"std": 0.5}) # GMY 1.0
    track_ang_vel_z_exp = RewTerm(func=mdp.track_ang_vel_z_world_exp, weight=1.0, params={"std": 0.5}) 
    lin_vel_z_l2 = RewTerm(func=mdp.lin_vel_z_l2, weight=-1.0)
    ang_vel_xy_l2 = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.05)
    energy = RewTerm(func=mdp.energy, weight=-1e-3) # GMY -1e-3
    dof_acc_l2 = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7) # GMY -2.5e-7
    action_rate_l2 = RewTerm(func=mdp.action_rate_l2, weight=-0.01)  # GMY -0.01   建议继续增大限制，到-0.05
    undesired_contacts = RewTerm(
        func=mdp.undesired_contacts,
        weight=-1.0,
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=["pelvis"]
            ),
            "threshold": 1.0,
        },
    )
    ...
    #     func=mdp.joint_deviation_standstill,
    #     weight=-0.2,
    #     params={
    #         "asset_cfg": SceneEntityCfg(
    #             "robot",
    ...
    hip_roll_action = RewTerm(func=mdp.hip_roll_action, weight=-0.1) # GMY -0.2
    hip_yaw_action = RewTerm(func=mdp.hip_yaw_action, weight=-0.1) # GMY -1.0
...
class TienKungWalkFlatEnvCfg:
    ...
    robot: RobotCfg = RobotCfg(
        ...
        action_scale=0.25,
        terminate_contacts_body_names=["pelvis"],
