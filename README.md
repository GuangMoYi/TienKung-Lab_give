...
If you encounter the following error when extracting or cloning on Windows:
```
Cannot create symbolic link: 客户端没有所需的特权。
```

**Solutions:**
1. **Extract as Administrator**: Right-click your extraction tool (WinRAR, 7-Zip, etc.) and select "Run as administrator", then extract again.
2. **Use Git with symlink disabled**: If using Git, configure it to not create symlinks:
   ```bash
   git config core.symlinks false
   git clone <repository-url>
   ```
3. **Verify the package**: Make sure you're using the latest version of the code package (symbolic links have been replaced with actual files).

You can verify there are no symlinks by running:
```bash
python check_symlinks.py
```

For more details, see [WINDOWS_COMPATIBILITY.md](WINDOWS_COMPATIBILITY.md).
